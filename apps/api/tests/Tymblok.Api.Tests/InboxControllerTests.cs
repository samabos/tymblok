...
public class InboxControllerTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        // Arrange
        var request = new CreateInboxItemRequest("Test Item", null, InboxPriority.Medium);
    ...
    public async Task CreateInboxItem_WithEmptyTitle_ReturnsBadRequest()
    {
        ...
        var request = new { title = "", description = "Test", priority = InboxPriority.Medium };
    ...
    public async Task GetInboxItems_WithAuth_ReturnsOk()
    {
        ...
        await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Item 1", null, InboxPriority.Medium));
    ...
    public async Task GetInboxItems_WithIsDismissedFilter_ReturnsFilteredResults()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("To Dismiss", null, InboxPriority.Medium));
        ...
        await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Active Item", null, InboxPriority.Medium));
    ...
    public async Task GetInboxItems_WithPriorityFilter_ReturnsFilteredResults()
    {
        ...
        await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Normal Priority", null, InboxPriority.Medium));
    ...
    public async Task GetInboxItem_WithValidId_ReturnsOk()
    {
        ...
        // Create an item
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Test Item", "Description", InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task GetInboxItem_NotOwnedByUser_ReturnsNotFound()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Private Item", null, InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task UpdateInboxItem_WithValidData_ReturnsOk()
    {
        ...
        // Create an item
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Old Title", "Old Description", InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task UpdateInboxItem_NotOwnedByUser_ReturnsForbidden()
    {
        ...
        // Create an item with first user
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Original", null, InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task DismissInboxItem_NotOwnedByUser_ReturnsForbidden()
    {
        ...
        // Create an item with first user
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Item", null, InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task DeleteInboxItem_WithValidId_ReturnsOk()
    {
        ...
        // Create an item
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("To Delete", null, InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
    ...
    public async Task DeleteInboxItem_NotOwnedByUser_ReturnsForbidden()
    {
        ...
        // Create an item with first user
        var createResponse = await _client.PostAsJsonAsync("/api/inbox", new CreateInboxItemRequest("Protected Item", null, InboxPriority.Medium));
        var createdItem = await createResponse.Content.ReadFromJsonAsync<ApiResponse<InboxItemDto>>();
